...
func KClient(bQuic bool, bUdp bool, bSctp bool, args []string) {
	...
	if bUdp {
		...
		if bQuic {
			...
			go clientLoop(conn, nil, nil, nil)
		} else {
			argsConnection = args[0]
			udpAdr, err := net.ResolveUDPAddr("udp", argsConnection)
			//log.Info().Str("arg0", args[0]).Msg("<< args[0]")
			...
			go clientLoop(nil, nil, connUDP, nil)
		}
		...
		log.Info().Str("IP:", tcpAdr.IP.String()).Str("Port", fmt.Sprint(tcpAdr.Port)).Msg("Connecting to remote SCTP peer")

		conn, err := sctp.DialSCTP("sctp", nil, sctpAddr)
		if err != nil {
			fmt.Printf("❌ Error al conectar con el servidor SCTP: %v", err)
			os.Exit(1)
		}
		//Cerrar diferido
		defer conn.Close()
		go clientLoop(nil, nil, nil, conn)
		...
		//Cerrar diferido
		defer conn.Close()
		go clientLoop(nil, conn, nil, nil)
	}
...
func clientLoop(connQuic *quic.Conn, connTCP *net.TCPConn, connUDP *net.UDPConn, connSCTP *sctp.SCTPConn) { //stream quic.Stream) {
	...
	for i := 1; true; i++ {
		...
		if connSCTP != nil {
			...
			// Read data from the client
			//readline for TCP
			connSCTP.SetReadDeadline(time.Now().Add(MAX_TIME_READ_DEADLINE))

			bytesReaded, err = connSCTP.Read(buf)
			if err != nil {
				//Log error
				log.Error().Msg(fmt.Sprintf("Error '%s'", err.Error()))
				chEndClient <- true //Notify client exit
