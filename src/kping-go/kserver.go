...
func kserver(args []string) error {
	...
	if err != nil {
		...
		log.Info().Msg(fmt.Sprintf("Starting kping IPv4-QUIC/UDP server on port %s", listenerQuic4.Addr().String()))
		nServers += 1
	}
	// IPv6 /////////////////////////////////////
	// UDP+QUIC ipv6 ////////////////////////////
	...
	if err6 != nil {
		...
		log.Info().Msg(fmt.Sprintf("Starting kping IPv6-QUIC/UDP server on port %s", listenerQuic6.Addr().String()))
		nServers += 1
	}
	...
	if errTCP != nil {
		...
		log.Info().Msg(fmt.Sprintf("Starting kping IPv4-TCP server on port %s", listenerTCP4.Addr().String()))
		nServers += 1
	}
	...
	tcpAdr, errSCTP := net.ResolveTCPAddr("tcp", "0.0.0.0:"+strconv.Itoa(iPort))
	//log.Info().Str("arg0", args[0]).Msg("<< args[0]")
	if errSCTP != nil {
		log.Error().Msg(fmt.Sprintf("Error %s", errSCTP.Error()))
		//return errSCTP
	}

	sctpAddr := &sctp.SCTPAddr{
		IPAddrs: []net.IPAddr{{IP: tcpAdr.IP}},
		Port:    tcpAdr.Port,
	}

	listenerSCTP4, errSCTP := sctp.ListenSCTP("sctp", sctpAddr)
	if errSCTP != nil {
		log.Error().Msg(fmt.Sprintf("Error %s", errSCTP.Error()))
		//return errTCP
	} else {
		log.Info().Msg(fmt.Sprintf("Starting kping IPv4-SCTP server on port %s", listenerSCTP4.Addr().String()))
		nServers += 1
	}
...
func listenIPv4UDP(iPort int, wg *sync.WaitGroup) {
	...
	log.Info().Msg(fmt.Sprintf("Starting kping IPv4-UDP server on port %d", iPort+1))
...
// Nueva conexión aceptada
func listenIPv4SCTP(listener *sctp.SCTPListener, wg *sync.WaitGroup) {
	defer wg.Done()

	for {
		connSCTP, err := listener.AcceptSCTP()
		if err != nil {
		...
		go handleSCTP4Connection(connSCTP)
	}
}
...
func handleQUICConnection(conn *quic.Conn) {
	...
	//Log error
	log.Info().Msg(fmt.Sprintf("Close connection QUIC client %s ", conn.RemoteAddr().String()))
...
func handleTCPConnection(conn net.Conn) {
	...
	//Log error
	log.Info().Msg(fmt.Sprintf("Close connection TCP client %s ", conn.RemoteAddr().String()))
