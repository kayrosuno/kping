...
//    
    ...
    func clientGUIHandleConnectionStateChanged(to state: NWConnection.State)  -> Void  {
        switch state {
        case .waiting(_):
            printGUI("\(TimeNow()) * connection state changed to WAITING state")
        case .failed(let error):
            printGUI("\(TimeNow()) * connection state changed to FAILED state")
            clientGUIConnectionFailed(error: error)
        case .ready:
            printGUI("\(TimeNow()) * connection state changed to READY state")
        case .setup:
            printGUI("\(TimeNow()) * connection state changed to SETUP state")
        case .preparing:
            printGUI("\(TimeNow()) * connection state changed to PREPARING state")
        default:
            break
        }
    }
    ...
    func clientGUIHandleReceiveData(
        ...
        if let data = content, !data.isEmpty {
            do {
                ...
                let rtt_time = TimeNowDouble() - Double(rtt_result.Time_client)
                let delay = Double(rtt_result.Time_server - rtt_result.Time_client)
                let now = TimeNow()
                
                //Visualizar el rtt como texto en la GUI
                //printGUI("\(now) id=\(rtt_result.Id)  RTT=\(rtt_time)us")
                printGUI(/*id: rtt_result.Id, */timeReceived: nowDouble, delay: delay, rtt: rtt_time, cadena: "\(now) id=\(rtt_result.Id) delay=\(delay)us RTT=\(rtt_time)us")
                //printDEBUG("\(now) id=\(rtt_result.Id)  RTT=\(rtt_time)us")
                
                //Visualizar el rtt actual
                cluster.actualRTTns = rtt_time
                
                //Visualizar el rtt
                cluster.actualRTTns = rtt_time
                if rtt_time > cluster.maxRTTns { cluster.maxRTTns = rtt_time }
                if rtt_time < cluster.minRTTns || cluster.minRTTns == 0 { cluster.minRTTns = rtt_time }
               
                //Media rtt
                let nuevaCantidadNumeros = Double (cluster.kpingDataChart.count + 1)
                cluster.medRTTns = cluster.medRTTns == 0 ? rtt_time :
                ((cluster.medRTTns * Double(cluster.kpingDataChart.count)) + rtt_time) / nuevaCantidadNumeros
...
//                id: 0,
    ...
    ///GENERAL:  function to print  to GUI
    @MainActor
    func printGUI(/*id: Int64,*/ timeReceived: Double, delay:Double, rtt: Double, cadena: String) {
